<?php
	...
	// Si no hay query, redirige directamente con error.
	if (empty($query)) {
		$error = "Please enter a search term.";
		redirectWithError($error, $query, $year, $gender, $continent, $country);
	}

	$searchTerm = "%$query%";

	// Búsqueda por disease
	...
	if (!$diseaseResult) {
		...
		// Búsqueda por pathogen si no hay disease
		...
		if (!$pathogenResult) {
			...
			// Redirige a temp_file.php como pathogen
			header("Location: temp_file.php?query=" . urlencode($query) . "&type=pathogen");
			exit();
		}
	} elseif ($diseaseResult->num_rows == 1) {
		// Redirige a temp_file.php como disease
	...
	function redirectWithError($error, $query, $year, $gender, $continent, $country) {
		$params = [
			'error' => urlencode($error),
			'query' => urlencode($query),
			'year' => urlencode($year),
			'gender' => urlencode($gender),
			'continent' => urlencode($continent),
